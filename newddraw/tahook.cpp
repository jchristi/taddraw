...
void CTAHook::CalculateRing()
{
	...
	int footx = TAdynmem->OwnUnitBegin[MouseOverUnit].UnitType->FootX + Spacing*2;
	int footy = TAdynmem->OwnUnitBegin[MouseOverUnit].UnitType->FootY + Spacing*2;
	int posx = TAdynmem->OwnUnitBegin[MouseOverUnit].XGridPos*16 - Spacing*16;
	int posy =  TAdynmem->OwnUnitBegin[MouseOverUnit].YGridPos*16 - Spacing*16;
...
void CTAHook::FindConnectedSquare(int &x1, int &y1, int &x2, int &y2, char *unittested)
{
	...
	while(XMatrix[i]!=-1 && YMatrix[i]!=-1)
	{
		...
		{
			int newx1 = TAdynmem->OwnUnitBegin[building].XGridPos*16;
			int newy1 = TAdynmem->OwnUnitBegin[building].YGridPos*16;
			int newx2 = newx1 + TAdynmem->OwnUnitBegin[building].UnitType->FootX*16;
			int newy2 = newy1 + TAdynmem->OwnUnitBegin[building].UnitType->FootY*16;
