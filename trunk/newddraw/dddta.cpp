...
void CDDDTA::Blit(LPDIRECTDRAWSURFACE DestSurf)
{
	...
	int OwnUnitBegin = FindMouseUnit();

	wsprintf(Outstr, "%i", OwnUnitBegin);	
...
void CDDDTA::WriteUnits()
{
	for(int player=0; player<DDDSharedMem->numPlayers; player++)
	{
		...
		{
			if(TAdynmem->Players[player].Units[i].UnitType)  //OwnUnitBegin active
			{
...
void inline CDDDTA::SetActiveUnit(OwnUnitBegin *DestUnit, UnitStruct *TAUnit)
{
	DestUnit->active = 1;
