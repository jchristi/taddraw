		...
		while (Counter<MaxHotUnitCount_I)
		{
			...
			if (0!=(UnitValid2_State& Current->UnitSelected))
			{
				if (0.0F==(Current->Nanoframe))
				{
					...
					if ((NULL!=Current->ValidOwner_PlayerPtr)&&(Player_LocalHuman==Current->ValidOwner_PlayerPtr->My_PlayerType))
					{
			/*			if (((NoWeaponPtr!=Current->UnitType->weapon1)|| 
							(NoWeaponPtr!=Current->UnitType->weapon2)||
							(NoWeaponPtr!=Current->UnitType->weapon3))
							)
						{
							DoSelect_b= true;
			...
			if (0.0F==(Current->Nanoframe))
			{
				...
				if ((NULL!=Current->ValidOwner_PlayerPtr)&&(Player_LocalHuman==Current->ValidOwner_PlayerPtr->My_PlayerType))
				{
					...
					if (0!=(WEAPONUNITS& NeededType)&&
							((NoWeaponPtr!=Current->UnitType->weapon1)|| 
							(NoWeaponPtr!=Current->UnitType->weapon2)||
							(NoWeaponPtr!=Current->UnitType->weapon3))
						)
					{
...
/*
					if (0!=(& NeededType)&&(0!=Current->UnitType->nWorkerTime)&&(0!=Curren))
					{
						DoSelect_b= true;
					}
*/
