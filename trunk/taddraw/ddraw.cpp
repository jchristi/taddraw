...
bool APIENTRY DllMain(HINSTANCE hinst, unsigned long reason, void*)
{
	...
	if(reason==DLL_PROCESS_ATTACH)
	{
		...
		DataShare->IsRunning = 5;
 		...
 		EnableSound();
		NowIncreaseAISearchMapEntriesLimit= new IncreaseAISearchMapEntriesLimit ( GetIniFileInt ( "AISearchMapEntries", 66650));
		NowIncreaseWeaponTypeLimit= new IncreaseWeaponTypeLimit (  GetIniFileInt ( "WeaponType", 0) );
		NowIncreaseUnitTypeLimit= new IncreaseUnitTypeLimit ( GetIniFileInt ( "UnitType", 0) );
		...
		NowSwitchalt= new Switchalt ( GetIniFileInt ( "SwitchAlt", 1));
		NowIncreaseSfxLimit= new IncreaseSfxLimit ( GetIniFileInt ( "SfxLimit", 0x5000));
		//NowIncreaseUnitLimit
